...
export class TaskItemComponent implements OnInit {
  ...
  onSendMessage(item) {
    Swal.fire({
      ...
      preConfirm: (note) => {
        const listPhone =
          this.data.msisdn.msisdns.length > 1
            ? this.data.msisdn.msisdns
                .map((x) => {
                  return x.msisdn;
                })
                .join("-")
            : this.data.msisdn.msisdns[0].msisdn;
  ...
  async onUpdateStatusV2(item, status) {
    if (
      status == this.taskTelecomStatus.STATUS_REJECT ||
      status == this.taskTelecomStatus.STATUS_CANCEL
    ) {
      let titleS;
      ...
      Swal.fire({
        ...
        preConfirm: (note) => {
          ...
          this.telecomService
            .updateTaskStatusV2(item.action, item.id, {
              status: status,
              note: note,
            })
            .subscribe(
      ...
      if ((await this.alertService.showConfirm(confirmMessage)).value) {
        this.telecomService
          .updateTaskStatusV2(item.action, item.id, { status: status })
          .subscribe(
            (res) => {
              if (!res.status) {
                this.alertService.showError(res.message);
  ...
  async onUpdateStatus(item, status) {
    if (
      ...
      if (status == this.taskTelecomStatus.STATUS_PROCESSING) {
        ...
        confirmMessage =
          "Xác nhận đã đẩy thông tin " + this.actionText + " sang nhà mạng?";
        ...
        confirmMessage =
          "Xác nhận đã " + this.actionText + " thành công 1 phần?";
      }

      if ((await this.alertService.showConfirm(confirmMessage)).value) {
        this.telecomService
          .updateTaskStatus(item.id, { status: status })
          .subscribe(
            (res) => {
              if (!res.status) {
                this.alertService.showError(res.message);
  ...
  async asyncToMnoViaApi(item) {
    ...
    if (item.action == this.listTaskAction.convert_info.value) {
      confirmMessage = "Xác nhận chuyển đổi chủ quyền TTTB";
    }
    if ((await this.alertService.showConfirm(confirmMessage)).value) {
      this.sectionBlockUI.start();
      this.telecomService.asyncToMnoViaApi(item).subscribe(
        (res) => {
          this.sectionBlockUI.stop();
          if (!res.status) {
            this.alertService.showError(res.message, 30000);
  ...
  async sendCallback(item) {
    this.telecomService.sendCallback(item).subscribe(
      (res) => {
        if (!res.status) {
          this.alertService.showError(res.message);
          return;
    ...
    if (status == this.msisdnStatus.STATUS_PROCESSED_MNO_FAIL) {
      Swal.fire({
        ...
        preConfirm: (note) => {
          ...
          this.telecomService
            .updateMsisdnStatus(item.id, dataUpdateMsisdn)
            .subscribe(
              (res) => {
                if (!res.status) {
                  Swal.showValidationMessage(res.message);
  ...
  async onUpdateStatus2G(item, status) {
    if (status == this.taskTelecomStatus.STATUS_CANCEL) {
      ...
      if ((await this.alertService.showConfirm(confirmMessage)).value) {
        this.telecomService
          .conversion2GApprove({ task_id: item.id, status: status })
          .subscribe(
            (res) => {
              if (!res.status) {
                this.alertService.showError(res.message);
  ...
  async onSelectFileSignature(event, file_type) {
    if (event.target.files && event.target.files[0]) {
      ...
      this.dataCreateSignature.people.identification_signature_file =
        this.imageSgnatureBase64.replace("data:image/png;base64,", "");
    }
  }
  ...
  async onSubmitUpload() {
    if (
      (await this.alertService.showConfirm("Bạn có muốn tải hình ảnh này lên?"))
        .value
    ) {
      this.sectionBlockUI.start();
      this.telecomService
        .patchSignature(this.item.id, this.dataCreateSignature)
        .subscribe(
          (res) => {
            this.sectionBlockUI.stop();
            if (!res.status) {
    ...
    if (
      (
        await this.alertService.showConfirm(
          "Bạn có đồng ý tạo phiếu yêu cầu/hợp đồng ?"
        )
      ).value
    ) {
      this.telecomService.getCreatContract(this.item.id).subscribe(
        (res) => {
          if (!res.status) {
            this.alertService.showMess(res.message);
            return;
  ...
  async onSubmitUploadSimInfo(serial) {
    ...
    if ((await this.alertService.showConfirm("Bạn có đồng ý lưu lại")).value) {
      ...
      this.telecomService.uploadSimInfo(data).subscribe(
        (res) => {
  ...
  async onResendMail() {
    ...
    if (
      (
        await this.alertService.showConfirm(
          `Bạn có đồng ý gửi lại vào email ${this.dataResendMail.email}`
        )
      ).value
    ) {
      this.telecomService.resendMail(this.dataResendMail).subscribe(
        (res) => {
          if (res && !res.status) {
            this.alertService.showMess(res.message);
            return;
  ...
  async onRetryTask() {
    if (
      (await this.alertService.showConfirm(`Bạn có đồng ý thực hiện lại?`))
        .value
    ) {
      const data = {
        task_id: this.data.task.id,
      };
      this.telecomService.retryTask(data).subscribe(
        (res) => {
          if (res && !res.status) {
            this.alertService.showMess(res.message);
            return;
  ...
  onViewImage(modal, type, mobile = null) {
    if (type == "cccd_front") {
      this.viewImage =
        ...
        this.data.customer.people.identification_front_file;
    }
    if (type == "cccd_back") {
      this.viewImage =
        "data:image/png;base64," +
        this.data.customer.people.identification_back_file;
    }
    if (type == "selfie") {
      this.viewImage =
        "data:image/png;base64," +
        this.data.customer.people.identification_selfie_file;
    }
    if (type == "signature") {
      this.viewImage =
        "data:image/png;base64," +
        this.data.customer.people.identification_signature_file;
    }
    if (type == "cccd_front_compare") {
      this.viewImage =
        "data:image/png;base64," + this.data.people.base64FrontCompare;
    }
    if (type == "cccd_back_compare") {
      this.viewImage =
        "data:image/png;base64," + this.data.people.base64BackCompare;
    }
    if (type == "selfie_compare") {
      this.viewImage =
        "data:image/png;base64," + this.data.people.base64SelfieCompare;
    }
    if (type == "signature_compare") {
      this.viewImage =
        "data:image/png;base64," + this.data.people.base64SignatureCompare;
    }
    if (type == "sim") {
      this.viewImage =
        "data:image/png;base64," + this.data?.msisdn?.base64SimFile[mobile];
    }
    if (type == "sim_compare") {
      this.viewImage =
        "data:image/png;base64," + this.data?.msisdn?.base64SimFile[mobile];
  ...
  isValidAsyncBy(task) {
    if (
      this.data.task.sub_action ==
        this.listTaskAction["_4G_VNM_TO_VMS"]["value"] &&
      task.status != TaskTelecomStatus.STATUS_SUCCESS
    )
      return true;
    return (
      this.currentUserId == task.sync_by &&
      task.status != TaskTelecomStatus.STATUS_SUCCESS &&
      this.typeDetail != "msisdn"
    );
  }
  ...
  isShowButtonApprove() {
    if (
      (this.checkAction("telecom-admin/task/approve-2g-conversion") &&
        this.data.task.sub_action ==
          this.listTaskAction["TYPE_2G_CONVERSION"]["value"]) ||
      this.data.task.sub_action ==
        this.listTaskAction["GSIM_TO_SIM"]["value"] ||
      this.data.task.sub_action ==
        this.listTaskAction["_4G_VNM_TO_VMS"]["value"]
    ) {
      return true;
  ...
  checkAction(item) {
    return this.listCurrentAction
      ? this.listCurrentAction.find((itemX) => itemX.includes(item))
      : false;
  }
  ...
  allowChangeCmndToCccd(task) {
    return (
      [
        ...
        TaskTelecomStatus.STATUS_APPROVED,
        TaskTelecomStatus.STATUS_WAITING_SIM,
      ].includes(task.status) &&
      ["GSIM_TO_SIM", "2G_CONVERSION", "4G_VNM_TO_VMS"].includes(
        task.sub_action
      ) &&
      this.checkAction("telecom-admin/task/2g-convert-cmnd-to-cccd")
    );
  }
  ...
  checkMsisdnTaskGsim(data) {
    return data.task.action == this.listTaskAction.CANCEL_SUBCRIBER.value &&
      data.msisdn.msisdns[0] &&
      data.msisdn.msisdns[0].mno == "GSIM"
      ? true
      : false;
  }
}
