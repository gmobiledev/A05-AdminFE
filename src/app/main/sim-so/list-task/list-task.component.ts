...
export class ListTaskComponent implements OnInit {
  ...
  constructor(
    ...
    this.activeRouted.queryParams.subscribe((params) => {
      ...
      this.searchForm.payment_gateway =
        params["payment_gateway"] && params["payment_gateway"] != undefined
          ? params["payment_gateway"]
          : "";
      this.searchForm.mobile =
        params["mobile"] && params["mobile"] != undefined
          ? params["mobile"]
          : "";
      this.searchForm.cccd =
        params["cccd"] && params["cccd"] != undefined ? params["cccd"] : "";
      this.searchForm.customer_name =
        params["customer_name"] && params["customer_name"] != undefined
      ...
      this.searchForm.status =
        params["status"] && params["status"] != undefined
          ? params["status"]
          : "";
      this.searchForm.mine =
        params["mine"] && params["mine"] != undefined ? params["mine"] : "";
      ...
      this.searchForm.date_range =
        params["date_range"] && params["date_range"] != undefined
          ? params["date_range"]
          : "";
      this.searchForm.array_status =
        params["array_status"] && params["array_status"] != undefined
          ? params["array_status"]
          : [];
      this.searchForm.sub_action =
        params["sub_action"] && params["sub_action"] != undefined
          ? params["sub_action"]
          : "";
      this.searchForm.payment_reference_number =
        params["payment_reference_number"] &&
        params["payment_reference_number"] != undefined
          ? params["payment_reference_number"]
          : "";
      this.searchForm.reference_id =
        params["reference_id"] && params["reference_id"] != undefined
          ? params["reference_id"]
          : "";
      this.initActiveBoxSummary();
      if (this.searchForm.action && this.searchForm.array_status.length > 0) {
        this.setActiveBoxSummary(
          this.searchForm.array_status,
          this.searchForm.action
        );
      }
    ...
    if (item) {
      ...
      if (item.customer_id && item.customer_type == "ORGANIZATION") {
        if (
          ...
          (this.checkAction("telecom-admin/task/:slug(\\d+)/update-status") ||
            this.checkAction(
              "telecom-admin/task/:slug(\\d+)/" + item.action + "/update-status"
            ))
        ) {
          try {
      ...
      if (
        ...
        try {
          ...
          if (
            (item.status != this.taskTelecomStatus.STATUS_NEW_ORDER &&
              this.checkAction("telecom-admin/task/approve-2g-conversion")) ||
            ((item.status == this.taskTelecomStatus.STATUS_NEW_ORDER ||
              item.status == this.taskTelecomStatus.STATUS_PROCESSING) &&
              (this.checkAction(
                "telecom-admin/task/:slug(\\d+)/update-status"
              ) ||
                this.checkAction(
                  "telecom-admin/task/:slug(\\d+)/" +
                    item.action +
                    "/update-status"
                )))
          ) {
  ...
  async paymentConfirm(id, status) {
    ...
    if ((await this.alertService.showConfirm(confirmMessage)).value) {
      this.telecomService.paymentConfirm(data).subscribe(
        (res) => {
          if (!res.status) {
  ...
  async paymentRefund(id) {
    ...
    if ((await this.alertService.showConfirm(confirmMessage)).value) {
      this.telecomService.paymentRefund(data).subscribe(
        (res) => {
          if (!res.status) {
            this.alertService.showError(res.message);
            return;
  ...
  async paymentRetry(id, status) {
    ...
    if ((await this.alertService.showConfirm(confirmMessage)).value) {
      this.telecomService.paymentRetry(data).subscribe(
        (res) => {
          if (!res.status) {
            this.alertService.showError(res.message);
            return;
  ...
  async paymentResend(id, newEmail = " ") {
    ...
    if ((await this.alertService.showConfirm(confirmMessage)).value) {
      this.telecomService.paymentResend(data).subscribe(
        (res) => {
          if (!res.status) {
            this.alertService.showError(res.message);
            return;
  ...
  async onSubmitViewMKH(taskId, status) {
    ...
    if ((await this.alertService.showConfirm(confirmMessage)).value) {
      this.gServiceService.lockService(taskId, status, "").subscribe(
        (res) => {
          if (!res.status) {
            this.alertService.showError(res.message);
            return;
  ...
  onSubmitExportExcelReport() {
    ...
    this.telecomService.exportExcelReport(this.searchForm).subscribe(
      (res) => {
  ...
  async setSubcribe() {
    ...
    getToken(messaging, { vapidKey: environment.FCM_VAPID_PUBLIC_KEY })
      .then((currentToken) => {
        if (currentToken) {
  ...
  getData() {
    ...
    this.telecomService.getAllTask(this.searchForm).subscribe(
      (res) => {
